...
func NewCdrReader(data []byte) (*CdrReader, error) {
	if len(data) < 4 {
		return nil, fmt.Errorf("invalid CDR data size %d, must contain at least a 4-byte header", len(data))
	}
...
func (r *CdrReader) Int16() (int16, error) {
	val, err := r.read(2)
	return int16(r.byteOrder().Uint16(val)), err
}
...
func (r *CdrReader) Int32() (int32, error) {
	val, err := r.read(4)
	return int32(r.byteOrder().Uint32(val)), err
}
...
func (r *CdrReader) Int64() (int64, error) {
	val, err := r.read(8)
	return int64(r.byteOrder().Uint64(val)), err
}
...
func (r *CdrReader) read(size int) ([]byte, error) {
	if r.offset+size > len(r.data) {
		return nil, fmt.Errorf("attempt to read past end of data (offset: %d, size: %d, data length: %d)", r.offset, size, len(r.data))
	}
