...
if [[ -z $REMOTE_SHA256 || -z $VERSION ]]; then
  echo "Error: Failed to extract SHA256 or Version from JSON file. Exiting."
  exit 1
fi
...
decompress_gz() {
    if command -v gzip &> /dev/null; then
        ...
        echo "Error: Neither gzip nor gunzip is available. Cannot decompress file."
        return 1
    fi
}
...
if decompress_gz "$TMP_FILE" "$TEMP_DIR/cos_binaries/cos/cos"; then
    ...
    echo "Failed to decompress cos binary. Exiting."
    exit 1
fi
