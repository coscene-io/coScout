...
def v2_spec_to_rules(project_rules_spec: dict, upload_fn: Callable, project_name: str) -> list[Rule]:
    ...
    for rule_idx, rule_spec in enumerate(project_rules_spec.get("rules", [])):
        ...
        for condition_spec in rule_spec.get("conditions", []):
            if "raw" in condition_spec:
                conditions.append(Condition(condition_spec["raw"]))
        ...
        actions: list[Action] = []
        for action_spec in rule_spec.get("actions", []):
            if "upload" in action_spec:
                actions.append(
                    Action("upload", action_spec["upload"], partial(upload_fn, rule=rule_spec, project_name=project_name))
                )
        if not actions:
            continue

        topics: list[str] = rule_spec.get("active_topics", [])

        scopes = rule_spec.get("scopes", [])
        if not scopes:
            scopes = [{}]
        for scope in scopes:
            rules.append(Rule(rule_spec, conditions, actions, scope, topics, rule_idx, {"project_name": project_name}))
    return rules
    ...
    for project_rule_sets in configs:
        ...
        for project_rule_set in project_rule_sets["rules"]:
            ...
            if project_rule_set.get("version", "") == "v1":
                ...
                rules = v2_spec_to_rules(project_rule_set, upload_fn, project_name)
